...
stories.md v1.3 の 17 ストーリー（タロウ 13 + ミナミ 5 - US-T05 統合済み = 17）+ ChatGPT GPT 関連 2 ストーリー（US-T11 / US-M03）を、**主担当 Unit と関与 Unit に分けてマッピング**する。

**マッピング方針（Q1=D ハイブリッド）**:
1. **状態を保持する Unit を主担当**（DynamoDB の書き込み主が最終的な責任を持つ）
2. **FR Coverage Matrix で確認**（components.md §4 と整合）
3. **Documentation (C8) は Unit 外**（Q3=C, Q3-α=A）
...
| US-T11 | ChatGPT 「タコ中 GPT」と会話してタコス文脈に引き戻される | **Documentation (C8)** | — | FR-6.3 |
...
| US-M03 | ChatGPT GPT の説教モード会話を共有する | **Documentation (C8)** | — | FR-6.3 |
...
**合計**: 17 ストーリー（v1.3 時点、US-T05 は v1.1 で US-T13 に統合済み）
...
### U7. PwaFrontend
- **主担当 1 件**: US-T14
- **関与（UI レイヤー）**: 全 17 ストーリー（PWA は全機能の入り口）

### U9. Infrastructure
- **主担当 0 件**（横断責務、ストーリー直接担当なし）

### Documentation（C8 ChatGPT GPT）
- **主担当 2 件**: US-T11, US-M03
...
- **US-T11, US-M03（Documentation セクション）**: ChatGPT GPT 公開、System Prompt 確定
- 全シナリオ磨き込み
- バイラル素材作成
...
| U7 | ⭕（時間支配 / UI 入口） |
| U9 | — |
| Documentation | ◎（会話の主導権喪失 / コンテンツ消費） |
...
✅ 17 ストーリー全てがマップ済み（17 = 13 タロウ + 5 ミナミ - US-T05 統合済み 1）
...
| US-M01〜M05 | 元ストーリーと同 Unit | ✅ |
| US-T11, US-M03 | Documentation | ⚠️ Unit 外 |

→ Q1=D（A+C ハイブリッド）の方針通り、状態を持つ Unit に集約され、U4 / U7 の "状態無し" ケースも FR で確認できているため健全。

### 6.3 ChatGPT GPT 関連ストーリーが Documentation に分離されているか（Q3=C 検証）

✅ US-T11 / US-M03 の主担当 = Documentation（Unit 外）。Iteration 4+ で別途進行。

---

## 7. 参照

- [unit-of-work.md](./unit-of-work.md) - 8 Unit + Documentation
- [unit-of-work-dependency.md](./unit-of-work-dependency.md) - Unit 間依存
- [../user-stories/stories.md](../user-stories/stories.md) - 元ストーリー集（v1.3）
