...
- 2026-05-08 v1.1（rework v2.0 復元）: ビジネス意図の深掘り rework に伴いバックアップから復元。Unit 依存関係は変化なし。
...
Unit Dependency Matrix（§1）の 8 ノード（U1〜U7, U9）が有向非巡回グラフ（DAG）を形成することを確認する。

> **v1.1 注**: 旧「Doc(C8)」ノードは FR-6.3 廃止に伴い削除済み。本検証は 8 ノードで実施する。

### 検証方法: トポロジカル順序の存在チェック

下記の処理順 `U9 → U1 → U2 → U3 → U4 → U5 → U6 → U7` を仮定したとき、各 Unit の依存先がすべて先行する Unit であれば循環依存なし（上三角行列のみに依存が存在する）。
...
**8 ノード DAG。循環依存ゼロ。** Walking Skeleton 戦略「U9 先行 → 全 Unit 並行実装」はトポロジカル順序として成立している。
