...
**Total Units**: 8（U1〜U7, U9）+ Documentation 1 セクション
**Decisions**: Q1=D / Q2=A / Q3=C / Q3-α=A / Q4=D / Q5=D

---

## 0. 設計方針サマリー

- **Unit 数**: 8 個（C1〜C7, C9 と 1:1）+ ChatGPT GPT は **Documentation セクション** に分離（Q3=C）
...
## 2. Documentation セクション（C8 ChatGPT GPT）

> **Q3=C により Unit から外す** → ドキュメント扱い

| 項目 | 内容 |
|------|------|
| **対象 Component（参考）** | C8 ChatGPTGptAsset |
| **位置付け** | Unit ではなく**ドキュメント / アセット作成タスク** |
| **責務** | ChatGPT カスタム GPT の System Prompt 設計 / 公開手順 / 共有リンク管理 |
| **主要 FR** | FR-6.3 |
| **関連 Story** | US-T11（GPT との会話） / US-M03（GPT 説教モード共有） |
| **コード位置** | `assets/prompts/tako-gpt-system-prompt.md`, `assets/prompts/tako-gpt-examples.md`, `assets/prompts/tako-gpt-publishing.md` |
| **テスト** | マニュアル確認（ChatGPT 上で Q-T11 / Q-M03 の受入基準シナリオを再現） |
| **Iteration 0 範囲** | 未着手（Iteration 4+ で着手） |
| **進行管理** | per-unit ループには乗せず、Iteration N の作業項目として個別管理 |
...
│   ├── prompts/                   # Documentation セクション（C8）
│   │   ├── tako-gpt-system-prompt.md
│   │   ├── tako-gpt-examples.md
│   │   └── tako-gpt-publishing.md
│   └── images/                    # SVG イラスト・通知画像
└── .github/
...
| Documentation (C8) | `assets/prompts/` |
...
| U7 | 1（US-T14） |
| U9 | 0（横断） |
| Documentation | 2（US-T11, M03） |
| **合計** | **18 = 17 + 1**（重複なし、US-T05 は v1.1 で US-T13 に統合済み） |

✅ 全 17 ストーリーがマップ済み。

### 5.2 全 9 Component / 8 Unit の責務明記確認
...
- ⚠️ U7（3 画面のみ）
- ❌ Documentation（C8 GPT は Iteration 4+）
