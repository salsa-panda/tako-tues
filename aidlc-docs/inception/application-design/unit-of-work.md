...
> 旧 C8（ChatGPT GPT Asset）は FR-6.3 廃止（要件 v1.8）に伴い削除済み。後続セクション番号は維持（§3 = コード組織戦略）。意思決定の詳細根拠は audit.md 参照。
...
| U7 | 2（US-T14, US-T15） |
| U9 | 0（横断） |
| **合計** | **17**（active: タロウ 13 + ミナミ 4。US-T11 / US-M03 削除済み、US-T14 / US-T15 追加済み） |

✅ 全 17 ストーリーがマップ済み（v2.0: US-T15「文化的受容」を U7 に追加）。
