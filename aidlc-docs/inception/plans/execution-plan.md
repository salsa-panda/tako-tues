...
| **Primary Changes** | ゼロからの新規構築（AWS サーバーレス + ChatGPT カスタム GPT 配布物） |
...
| 観点 | 該当 | 概要 |
|------|------|------|
| **User-facing changes** | ✅ Yes | PWA ダッシュボード、Web Push、ChatGPT GPT 経由のフル UX 設計 |
...
- [ ] **Application Design** — **EXECUTE**
  - **Rationale**: 新規コンポーネント／サービス境界（強制発注エンジン、刺激エージェント、配送モック、レシピライブラリ参照、Push エスカレーションカウンタ等）を 0 から定義する必要がある。コンポーネント間の責務分離、ビジネスルール、依存関係、外部統合点（Bedrock 煽り文、Web Push、ChatGPT GPT 配布、リポジトリ同梱レシピアセット）を明文化しないと Construction で迷走するリスクが高い。ハッカソン審査基準（ドキュメント品質・Unit 分解の適切さ）にも直結する。
- [ ] **Units Generation** — **EXECUTE（審査基準上の必須化）**
  - **Rationale**: OI-11 で記録の通り、ハッカソン審査では「**Unit 分解の適切さ**」が評価対象。書類審査（5/10）に向け、PoC を MVP として段階的に実装するため Unit に切り分ける必要がある。想定 Unit 例: (1) Auth & Trial、(2) Intake Logging、(3) Order Engine + Delivery Mock、(4) **Recipe Library (Pre-curated)**、(5) Stimulus Engine（Push + Escalation + Dashboard 煽り）、(6) Salsa & T-shirt Punishment、(7) PWA Frontend、(8) ChatGPT GPT Asset、(9) Infrastructure (CDK)。
...
- [ ] **Functional Design (per-unit)** — **EXECUTE**
  - **Rationale**: 強制発注の逆比例ロジック（FR-2.1）、24h ウィンドウ集計（FR-1.1, FR-6.4）、Type A/B 切替（FR-2.2）、説教モード切替（FR-6.3）など、複雑な業務ルールを宿す Unit が複数ある。データモデル（DynamoDB Single-table のキー戦略）も明確に必要。
...
| 7 | **U7: PWA Frontend (Dashboard + Countdown + Share)** | FR-4.1, FR-4.2, FR-4.3, FR-4.4 | US-T14, US-T10, US-M01〜M05 | ⚠️ 部分（3 画面のみ、Share は後回し） | API 契約で並行可能 |
| 8 | **U8: ChatGPT Custom GPT Asset** | FR-6.3 | US-T11, US-M03 | ❌ Iteration 0 では未着手（Q-α=C） | 完全独立（OpenAI 側） |
...
- Iteration 4: 「ChatGPT GPT 公開」→ U8 を初めて触る
- Iteration 5: 「Share 機能 + 動画字幕画像」→ U7 の磨き込み
...
| **M2: 予選 MVP** | 2026-05-30 | Iteration N まで反復済みの磨かれた MVP + ChatGPT GPT 公開 + 15 分プレゼン |
...
> **Q-α=C 反映**: ChatGPT GPT は Iteration 0 から切り離す。AWS 側の E2E に集中。
...
| **U8 ChatGPT GPT** | **Iteration 0 では未着手**（Q-α=C） | 全部 |
...
これだけ。Push エスカレーション・GPT・バリエーションウィーク・Share 機能は全て Iteration 1 以降。
...
| 期間 | 反復回数（目安） | 主な狙い |
|------|---------------|---------|
| **5/8〜5/27**（19 日） | **6〜9 サイクル** | E2E を磨き、Push エスカレーション・バリエーションウィーク・GPT 公開・Share 機能を順次足す |
...
> 「サイクル数 = バッファ」と捉えるのが v1.3 のコア。**動いているアプリを触ってからのフィードバック**を 12 回以上回せる前提で設計している。サイクル数が予定より少なくなりそうなら**スコープを削る**（例: GPT 公開は決勝までに延期、Share 機能は最低限）。サイクル数を維持するためにバッファを削るのは禁止。

### 6.6 PoC やりきり判定

「PoC として完成」と認める基準（Q3=A、§6.5 のサイクルで段階的に達成していく）:
...
- [ ] **GPT シナリオ**: ChatGPT カスタム GPT が公開状態で、3 ターン回避で説教モードに切り替わることを確認（Iteration 4 以降、書類審査前後に着手）
...
- ChatGPT カスタム GPT の System Prompt 草案（assets/prompts/）
- README に「コンセプト・スクショ風 UI モック・体験リンク（暫定）」を掲載

#### 予選 MVP 時点（5/30）
- AWS 上で稼働する MVP（U1〜U7 動作 + GPT 公開）
- E2E シナリオ: 強制発注 → 材料受領 → 24h カウントダウン → サルサ + Tシャツ罰 → 摂取記録によるリセット
- ChatGPT GPT を起動して 3 ターンで説教モード再現
