...
### Step A: 既存成果物の参照
- [ ] application-design.md v1.0 の §2 9 コンポーネント早見表
- [ ] execution-plan.md v1.3 §4 Suggested Units の Iteration 0 列
- [ ] component-dependency.md v1.0 の §1 Dependency Matrix
- [ ] services.md v1.0 の §1 Lambda 関数構成

### Step B: Unit 定義の確定
- [ ] 9 Unit を 1:1 で Component に対応（U1〜U9）
- [ ] 各 Unit の責務・含まれるストーリー・主要 FR を明記
- [ ] 各 Unit の Iteration 0 範囲を再掲

### Step C: 依存マトリクス
- [ ] Application Design の Dependency Matrix を Unit 視点に変換
- [ ] Walking Skeleton 立ち上げ時の必須依存パスを抽出
- [ ] 並行作業可能な Unit 群を識別

### Step D: ストーリーマッピング
- [ ] 17 ストーリー（タロウ 13 + ミナミ 5、UI/UX 関連で重複あり）を Unit にマップ
- [ ] 各ストーリーの主担当 Unit と関与する Unit を区別

### Step E: コード組織戦略（Greenfield）
- [ ] リポジトリディレクトリ構造を決定（モノレポ vs マルチリポジトリ等）
- [ ] Unit 毎のディレクトリ構成
- [ ] 共有コード（型定義、契約、ユーティリティ）の配置場所
...
### Step G: 完整性レビュー
- [ ] 全 17 ストーリーがいずれかの Unit にマップされている
- [ ] 全 9 Unit に責務・依存・コード位置が明記されている
- [ ] Walking Skeleton 立ち上げに必要な Unit セットが識別されている
