...
### Step A: 既存成果物の再読込
- [ ] requirements.md v1.7 の FR/NFR/Tech Stack 全項
- [ ] stories.md v1.3 の Acceptance Criteria（受入基準）
- [ ] story-board.md v1.3 のタイムラインと UI コピー
- [ ] execution-plan.md v1.3 の 9 Unit 暫定マッピング

### Step B: コンポーネント識別
- [ ] 9 Unit を「コンポーネント」に対応付け（1:1 とは限らない）
- [ ] 横断コンポーネント（Auth Context、Push Subscription Manager 等）の有無を判定
- [ ] 静的アセット（レシピ JSON、フォールバックテンプレート）の格納コンポーネント決定

### Step C: メソッドシグネチャ（高レベル）
- [ ] 各コンポーネントの主要メソッドの I/O 型を列挙
- [ ] DynamoDB アクセスパターンに直結するメソッドを優先抽出
- [ ] Bedrock 呼び出しメソッドの抽象化（フォールバック含む）

### Step D: サービス層 / オーケストレーション
- [ ] EventBridge Scheduler でトリガーされる **Scheduled Service**（金 21:00 / 月 21:00 / 火 21:00 / 平日 12:00 / 18:00）
- [ ] Lambda Handler の責務分離（API Gateway 経由 / EventBridge 経由）
- [ ] Single-table DynamoDB のキー戦略（PK/SK パターン）

### Step E: 依存・通信パターン
- [ ] Unit 間連携を Unit Coupling Principles P1（イベント駆動）に従い設計
- [ ] EventBridge カスタムバス上のイベント名を確定
- [ ] 同期通信が必要な箇所（PWA → API）と非同期通信（バックエンド間）を区別
...
- [ ] application-design.md（統合）生成

### Step G: 完整性レビュー
- [ ] 全 FR がいずれかのコンポーネントにマッピングされている
- [ ] 全 User Story の受入基準を満たすメソッドが定義されている
- [ ] Walking Skeleton で必要な最小契約が揃っている
- [ ] OI-14（レシピ JSON Schema）の方針が明文化されている
...
- **A. 9 Unit = 9 コンポーネントに 1:1 対応**（execution-plan の構成を維持）
