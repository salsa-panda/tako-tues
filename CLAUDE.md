...
## Project-Specific Rules（タコ中 / tako-tues）

### AI・Bedrock 利用範囲
- **Bedrock 用途を限定する**: FR-6.1（ダッシュボード煽り文）と FR-6.2（誘惑 Push 文）の動的生成のみ
- フォールバック静的テンプレートを必ず用意する（Bedrock 障害時も機能する）
- レシピ生成に Bedrock を使わない（食中毒リスク回避）
...
- AI-DLC 成果物の改訂はマイナー版 up（v1.x → v1.x+1）+ audit.md 記録 + ユーザー承認（Approval Final）を経てからコミット
